...
import type { CorrectionDetection } from './CorrectionDetection.js';
import {
    CorrectionDetectionFromJSON,
    CorrectionDetectionFromJSONTyped,
    CorrectionDetectionToJSON,
    CorrectionDetectionToJSONTyped,
...
export interface CorrectionAnalysis {
    ...
    /**
     *
     * @type {Array<CorrectionDetection>}
     * @memberof CorrectionAnalysis
     */
...
/**
 * Check if a given object implements the CorrectionAnalysis interface.
 */
export function instanceOfCorrectionAnalysis(value: object): value is CorrectionAnalysis {
    if (!('detections' in value) || value['detections'] === undefined) return false;
    return true;
}

export function CorrectionAnalysisFromJSON(json: any): CorrectionAnalysis {
    return CorrectionAnalysisFromJSONTyped(json, false);
}

export function CorrectionAnalysisFromJSONTyped(json: any, ignoreDiscriminator: boolean): CorrectionAnalysis {
    if (json == null) {
        return json;
    }
    return {

        'mealName': json['meal_name'] === undefined ? undefined : json['meal_name'] === null ? null : json['meal_name'],
        'totalNutrients': json['total_nutrients'] == null ? undefined : NutritionFactsFromJSON(json['total_nutrients']),
        'detections': ((json['detections'] as Array<any>).map(CorrectionDetectionFromJSON)),
    };
}

export function CorrectionAnalysisToJSON(json: any): CorrectionAnalysis {
    return CorrectionAnalysisToJSONTyped(json, false);
}

export function CorrectionAnalysisToJSONTyped(value?: CorrectionAnalysis | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }
    ...
    return {
        ...
        'detections': ((value['detections'] as Array<any>).map(CorrectionDetectionToJSON)),
    };
}
