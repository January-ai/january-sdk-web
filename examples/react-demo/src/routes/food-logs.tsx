...
import { CalendarDays, ChevronLeft, ChevronRight, Droplets, NotebookPen, Pencil, Plus, RefreshCw, Scale, Trash2, Utensils } from 'lucide-react'
...
export const Route = createFileRoute('/food-logs')({ component: LogsPage })
...
function LogsPage() {
  ...
  return (
    <Page data-testid="food-logs-screen">
      <PageHeader aside={<Button data-testid="food-log-add" disabled={!ready} onClick={() => openEditor()} type="button"><Plus aria-hidden="true" className="size-4" />Add meal</Button>} description="One partner-owned account ID and timezone scope every log. Pick a day: the SDK sends it as the inclusive start and end date for meals, water, and weight." eyebrow="Partner-owned identity" title="Your logs, one day at a time." />
        ...
        <div className="space-y-5 xl:sticky xl:top-8">
          ...
          <Card className="p-5 sm:p-6" data-testid="logs-day-card">
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              ...
              <SecondaryButton data-testid="food-logs-refresh" disabled={!ready} onClick={() => { invalidateFood(); queryClient.invalidateQueries({ queryKey: ['water-logs'] }); queryClient.invalidateQueries({ queryKey: ['weight-logs'] }) }} type="button"><RefreshCw aria-hidden="true" className="size-4" />Reload day</SecondaryButton>
            </div>
          </Card>
        </div>
        ...
        <div className="space-y-8">
          <section aria-busy={logs.isFetching} aria-live="polite" data-testid="food-section">
            <div className="mb-4 flex items-end justify-between gap-4">
              ...
              <NotebookPen aria-hidden="true" className="size-7 text-stone-400" />
            </div>
            ...
            {removeFood.isError ? <div className="mb-4"><ErrorMessage error={removeFood.error} testId="food-log-delete-error" /></div> : null}
            {!ready ? <EmptyState description="Save an active user to load the day's meals, water, and weight." icon={<NotebookPen aria-hidden="true" className="size-6" />} testId="food-logs-prompt" title="No user yet" />
              : logs.isPending && !logs.data ? <SkeletonList testId="food-logs-loading" />
                : logs.isError ? <ErrorMessage error={logs.error} testId="food-logs-error" />
                  : logs.data?.items.length ? <div className="space-y-4" data-testid="food-log-list">{logs.data.items.map((log, index) => <Card className="overflow-hidden" data-testid={`food-log-${index}`} key={log.id ?? `log-${index}`}>
                    ...
                    : <EmptyState description="No meals were logged for this person on this day." icon={<NotebookPen aria-hidden="true" className="size-6" />} testId="food-logs-empty" title="No food logs found" />}
          </section>
