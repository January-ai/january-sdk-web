...
export function ResultRow({ media, title, meta, onClick, busy = false, disabled = false, testId }: { media: ReactNode; title: string; meta: string; onClick?: () => void; busy?: boolean; disabled?: boolean; testId?: string }) {
  ...
  return onClick ? <button className="flex min-h-24 w-full items-center gap-4 border-b border-stone-200 px-5 py-4 text-left last:border-0 hover:bg-stone-50 disabled:cursor-wait disabled:opacity-70" data-testid={testId} disabled={disabled || busy} onClick={onClick} type="button">{content}</button> : <div className="flex min-h-24 items-center gap-4 border-b border-stone-200 px-5 py-4 last:border-0" data-testid={testId}>{content}</div>
}
