...
export function Button({ className, children, busy = false, busyTestId, ...props }: ButtonHTMLAttributes<HTMLButtonElement> & { busy?: boolean; busyTestId?: string }) {
  return (
    <button className={cn('inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-stone-950 px-5 text-sm font-bold text-white shadow-sm hover:bg-stone-800 disabled:cursor-not-allowed disabled:bg-stone-200 disabled:text-stone-500', className)} {...props}>
      {busy && <LoaderCircle aria-hidden="true" className="size-4 animate-spin motion-reduce:animate-none" data-testid={busyTestId} />}
      {children}
    </button>
  )
}
