...
// Water and food logs created here are deleted at the end; the API cannot delete weight logs.
...
test.describe('Live demo @live', () => {
  ...
  test('Baseline: the QA user as the API holds it @catalog @logs', async ({ page }) => {
    await step(page, 'Baseline', 'Read the QA user\'s logs for today and yesterday', async () => {
      if (!run.baseline) {
        run.baseline = {
          today: { water: { fl_oz: await waterTotal(today, 'fl_oz') }, weight: await weightOn(today), meals: (await foodLogs(today)).length },
  ...
  test('Food logs: create, list, edit, summary, and delete @logs', async ({ page }) => {
    let lunchId = ''
    let snackId = ''
    ...
    await step(page, 'Food logs', `Create "${lunch}" with banana and apple`, async () => {
      ...
      const row = page.getByTestId(/^food-log-\d+$/).filter({ hasText: lunch })
      ...
      verified('Created meal', await row.textContent(), { id: created.id, name: created.name, foods: created.foods.map((food: { name: string; quantity: number }) => `${food.quantity} × ${food.name}`) })
    }, () => page.getByTestId(/^food-log-\d+$/).filter({ hasText: lunch }))
    await step(page, 'Food logs', 'Edit it: rename, and 1.5 apples', async () => {
      const row = page.getByTestId(/^food-log-\d+$/).filter({ hasText: lunch })
      await row.getByTestId('food-log-edit').click()
      ...
      await byId(page, 'food-log-save').click()
      await expect(byId(page, 'food-log-editor')).toBeHidden()
      await expect(page.getByTestId(/^food-log-\d+$/).filter({ hasText: `${lunch} (edited)` })).toBeVisible()
      ...
      verified('Edited meal', `${lunch} (edited)`, { name: server.body.name, quantities: server.body.foods.map((food: { quantity: number }) => food.quantity) })
    }, () => page.getByTestId(/^food-log-\d+$/).filter({ hasText: lunch }))
    await step(page, 'Food logs', 'Tracking shows the meal and the day\'s totals from the summary', async () => {
      await openDemo(page, '/tracking')
      await expect(page.getByTestId(/^tracking-meal-\d+$/).filter({ hasText: `${lunch} (edited)` })).toBeVisible()
      ...
      verified('Day totals', await text(page, 'food-day-totals'), summary.totals)
      await expect(page.getByTestId(/^tracking-meal-\d+$/)).toHaveCount((await foodLogs(today)).length)
    ...
    await step(page, 'Food logs', `Add "${snack}" from Tracking`, async () => {
      ...
      await byId(page, 'food-log-save').click()
      await expect(byId(page, 'food-log-editor')).toBeHidden()
      await expect(page.getByTestId(/^tracking-meal-\d+$/).filter({ hasText: snack })).toBeVisible()
    ...
    await step(page, 'Food logs', 'Delete the lunch on Logs', async () => {
      ...
      await byId(page, 'food-logs-refresh').click()
      const row = page.getByTestId(/^food-log-\d+$/).filter({ hasText: lunch })
      await row.getByTestId('food-log-delete').click()
      await expect(page.getByTestId(/^food-log-\d+$/).filter({ hasText: lunch })).toHaveCount(0)
      const server = await foodLog(lunchId)
      expect(server.status).toBe(404)
      expect((await foodLogs(today)).some((log: { id: string }) => log.id === lunchId)).toBe(false)
  ...
  test('Water in fl oz and ml, delete the last entry, and an earlier day @logs', async ({ page }) => {
    ...
    await step(page, 'Water', 'Log 8 fl oz', async () => {
      ...
      rememberCreated('water', (await byId(page, 'water-log-last').getAttribute('data-log-id'))!, '8 fl oz today')
    ...
    await step(page, 'Water', 'Log 250 ml', async () => {
      ...
      rememberCreated('water', (await byId(page, 'water-log-last').getAttribute('data-log-id'))!, '250 ml today')
      ...
      verified('Water today after 250 ml', await text(page, 'water-day-total'), { ml: server, fl_oz: await waterTotal(today, 'fl_oz') })
    }, () => byId(page, 'water-section'))
    ...
    await step(page, 'Water', 'Log 12 fl oz on yesterday, then delete it', async () => {
      await byId(page, 'water-unit-fl-oz').click()
      await byId(page, 'logs-day-previous').click()
      await expect(byId(page, 'logs-day-label')).toHaveText('Yesterday')
      const before = await waterTotal(yesterday, 'fl_oz')
      await byId(page, 'water-amount').fill('12')
      await byId(page, 'water-log-add').click()
      await expect(byId(page, 'water-log-last')).toContainText('Logged 12 fl oz on')
      rememberCreated('water', (await byId(page, 'water-log-last').getAttribute('data-log-id'))!, '12 fl oz yesterday')
      await expect(byId(page, 'water-day-total')).toHaveText(`${shown(before + 12)} fl oz`)
      const onYesterday = await waterTotal(yesterday, 'fl_oz')
      const onToday = await waterTotal(today, 'fl_oz')
      expect(onYesterday).toBeCloseTo(before + 12, 1)
      expect(onToday).toBeCloseTo(flOz, 1)
      verified('Logged while viewing yesterday lands on yesterday', await text(page, 'water-day-total'), { yesterday: onYesterday, today: onToday })
      await byId(page, 'water-log-delete').click()
      await expect(byId(page, 'water-log-last')).toHaveCount(0)
      expect(await waterTotal(yesterday, 'fl_oz')).toBeCloseTo(before, 1)
    }, () => byId(page, 'water-section'))
  })
  ...
  test('Weight in lb and kg @logs', async ({ page }) => {
  ...
  test('Charts: week, month, and year against the API @logs', async ({ page }) => {
    const ranges = {
    ...
    for (const [range, { start, slots }] of Object.entries(ranges)) {
      await step(page, 'Charts', `Water, ${range}`, async () => {
        ...
        // The demo asks in chunks of at most 90 days; the API is asked for the same span here.
        const chunks: Array<{ date: string; total: { value: number } }> = []
        for (let from = start; from <= today; ) {
          const [year, month, day] = from.split('-').map(Number)
          const to = localDay(89, new Date(year!, month! - 1, day!, 12))
          chunks.push(...await waterTotals(from, to < today ? to : today, 'fl_oz'))
          from = localDay(90, new Date(year!, month! - 1, day!, 12))
        }
        const total = chunks.reduce((sum, item) => sum + item.total.value, 0)
        const logged = range === 'year' ? new Set(chunks.map((item) => item.date.slice(0, 7))).size : chunks.length
        if (logged) {
      ...
      await step(page, 'Charts', `Weight, ${range}`, async () => {
        ...
        const items = await weights(start, today)
        if (items.length) {
        ...
        verified(`Weight chart, ${range}`, items.length ? await text(page, 'weight-chart-latest') : 'No weight logged in this range', { entries: items.length, latest: items.at(-1)?.weight ?? null })
      }, () => byId(page, 'weight-section'))
    }
  })

  test('Cleanup: delete the water and food logs this run created @catalog @logs', async ({ page }) => {
    await step(page, 'Cleanup', 'Delete every water and food log in the ledger', async () => {
      setLogContext('cleanup')
      const results = await cleanup()
      observed('Deleted', results)
      // Logs deleted in the demo already are gone: water deletes succeed again, food answers 404.
      ...
      expect(await waterTotal(today, 'fl_oz')).toBeCloseTo(baseline.today.water.fl_oz, 1)
      expect(await waterTotal(yesterday, 'fl_oz')).toBeCloseTo(baseline.yesterday.water.fl_oz, 1)
      verified('After cleanup', { today: await state(today) }, baseline)
    })
  })
})
